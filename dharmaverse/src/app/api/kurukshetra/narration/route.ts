...
export async function POST(req: Request) {
  try {
    ...
    const systemPrompt = contextAssembler.assembleWarDayPrompt(warDayId, daySummary, narrativeContext);

    const result = streamText({
      model: google('models/gemini-2.5-flash'),
      system: systemPrompt,
      prompt: "Provide the cinematic narration for this day.",
    });
